...
const GeneratorSidePanel = ({records, selectedName, onSelectRecord, onSubmit, onDelete}) => {
  return (
    <div className="container--center">
      ...
      />
      <Text placeholder='Name' selected={selectedName} onChange={({ selected }) => { onSelectRecord(selected); }} />
      <Button classColor='primary' label="Save" onClick={() => { onSubmit(selectedName);}} />
      <Button classColor='primary' label="Delete" onClick={() => { onDelete(selectedName);}} />
    </div>
  );
};
