...
const getSvgComponents = (data) => {
  ...
  const svgComponents = lines
    ...
    .splice(ONE)
    .map((currentSegment) => {
      const dashCaseName = getAttributeList(currentSegment, 'data-testid="component-')
        [ZERO].replace('data-testid="component-', '')
        .replace('"', '');
...
const getParsedComponents = (data) => {
  ...
  return componentsWithSubcomponents.map((entry) => {
    ...
    return {
      ...entry,
      component: updatedComponent.updatedSvgObj,
      conditions: updatedComponent.conditions,
      subcomponents: updatedSubcomponent
    };
  });
};
...
const getParsedData = (data) => {
  return data.map((entry) => {
    const subcomponentTestData = entry.subcomponents
      ? entry.subcomponents
        .map((item) => {
          return `{ component:'${item.name}', transform:'translate(0,0)', conditions:{${
            item.conditions ? item.conditions.map((cond) => `${cond}:true`).join(',') : ''
          }}}`;
        })
        .join(',')
      : '';

    const reactSubcomponents = entry.subcomponents
      ? entry.subcomponents
        .map((item) => {
          return subcomponentTemplate
            ...
            .replace(
              '{{conditions}}',
              item.conditions.length > ZERO
                ? `const {${item.conditions.map((cond) => ` ${cond}`).join(',')} } = conditions;`
                : ''
            );
        })
        .join('\n')
    ...
    const reactComponent = reactSubcomponents
      ? componentTemplate
        ...
        .replace(
          '{{conditions}}',
          entry.conditions.length > ZERO
            ? `const {${entry.conditions.map((cond) => ` ${cond}`).join(',')} } = conditions;`
            : ''
        )
        .replace(/\n{3,}/g, '\n\n')
      : componentWithoutSubcomponentTemplate
    ...
    return {
      name: entry.name,
      subcomponentNames: entry.subcomponents.map((item) => item.name),
      reactComponent,
      conditions: entry.conditions,
      subcomponentTestData
    };
  });
};

const getGeneratedContent = (svgTagAttributes, data) => {
  const constContent = data
    .map((entry) => {
      const subcomponentReactNames = entry.subcomponentNames
        .map((name) => {
          return `  ${name}SVG`;
        })
        .join(',\n');
      const subcomponentsconst = subcomponentReactNames ? `,\n${subcomponentReactNames}` : '';

      return constTemplate
        .replace('{{consts}}', `${entry.name}SVG${subcomponentsconst}`)
  ...
  const svgMapContent = data.map((entry) => {
    ...
    const subcomponentNamesKeyMap = entry.subcomponentNames
      .map((name) => {
  ...
  const indexContent = indexTemplate
    .replace('{{consts}}', constContent)
    .replace('{{svgMap}}', `const svgMap = {\n${svgMapContent}};`);

  const svgMapperContent = svgMapperTemplate.replace('{{svgTagAttributes}}', svgTagAttributes);
