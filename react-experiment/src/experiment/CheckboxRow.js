...
const CheckboxRow = ({ id = '', label = '', values = [], selectAllLabel = '', selectAll = false, onChangeRow }) => {
  ...
  return (
    <Fragment>
      <label>{label}</label>
      <CheckboxRenderer id={id} label={selectAllLabel} selected={selectAll} onChangeCheckbox={handleSelectAllChange} />
      <CheckboxList values={values} onChangeCheckboxList={handleCheckboxRowChange} />
    </Fragment>
  );
};
